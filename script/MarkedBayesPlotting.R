...
# Answer:
# #TEMPERATURE SEEMS TO DECREASE MORE SIGNIFICANTLY IN M2, BUT EFFORT AND WIND SEEM TO BE ABOUT THE SAME.

# Choose model going forward
mr <- m2  # Use m2 as the chosen model
...
# If the model does not rely on data augmentation 
for (i in 1:I) {
  N_df[i, 'N.mean'] <- mean(mr$sims.list$N[, i])
  N_df[i, c('N.lowCI', 'N.highCI')] <- as.vector(quantile(
    mr$sims.list$N[, i], probs = c(0.025, 0.975)
...
# Plot abundance between sites and years
N_df %>% 
  ggplot(., aes(y = SiteID, x = N.mean, color = as.factor(year))) +
  theme_classic(base_size = 12) +
  theme(legend.position = "top") +
  labs(x = "Annual abundance", y = "Site") +
  geom_point(size = 3) +
  geom_errorbar(aes(xmin = N.lowCI, xmax = N.highCI), size = 2)
