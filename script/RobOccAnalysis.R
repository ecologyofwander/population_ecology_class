...
#answer: YES BECAUSE EACH SURVEY HAS ITS ON ROW WITHIN EACH YEAR.
...
# Answer:
# BECUASE THIS IS THE NAIVE MODEL WITH EVERYTHING IN IT. PSI IS GOING TO BE EXTINCTION AND COLONIZATION, NOT THE PREDICATORS.
# View model results:
fm.dyn
...
# Answer:
#THIS GIVES US THE VALUES BEYOND THE EFFECT SIZE, THE CONFIDENCE INTERVALS.
...
# Answer:
#EXTINCTION IS MORE LIKELY WITH CHEATGRASS AND COLONIZATION MORE LIKELY WITH SAGEBRUSH. THE EFFECT SEEMS STRONGER FOR SAGEBRUSH.
# Estimate confidence intervals for coefficients for detection submodel:
confint( fm.dyn, type = 'det' )
# How do we interpret these results?
# Answer:
#SAGEBRUSH SEEMS TO HAVE A VALUE OVER ZERO, WHICH JUST NOT SUGGEST MUCH STRENGTH TO THE MODEL IN REGARDS TO SAGEBRUSH AND DETECTION.
# How are they different to our single-season detection model?
# Answer:
#COLONIZATION AND EXTINCTION IS PSI VERSUS WITH SINGLE SEASON WHICH OCUPANCY HAS ITS OWN FORMULA.
##########################################################################
...
pred_det <- predict(fm.dyn, type = "det", newdata = umf, appendData = TRUE)

detection_plot <- cbind(pred_det[, c("Predicted", "lower", "upper")], sagebrush) %>%
  ggplot(., aes(x = sagebrush, y = Predicted)) + 
  theme_bw(base_size = 15) +
  labs(x = "Sagebrush (%)", y = "Predicted Detection Probability") +
  geom_smooth(aes(ymin = lower, ymax = upper), 
              stat = "identity",
              linewidth = 1.5, alpha = 1, color = "grey") + 
